    ...
    const farms = await Farm.findAll({
      ...
      include: [
        ...
        },
        {
          ...
          as: 'farmInstallmentOwnerships',
          attributes: ['id', 'createdAt'],
          required: false
        },
        {
          model: FarmUnitOwnership,
          as: 'farmUnitOwnerships',
          attributes: ['id', 'createdAt'],
          required: false
        }
      ],
      ...
      farms.map(async (farm) => {
        try {
          ...
          const purchaseDate = farm.Transactions?.[0]?.transaction_date || 
                             farm.farmInstallmentOwnerships?.[0]?.createdAt ||
                             farm.farmUnitOwnerships?.[0]?.createdAt ||
                             farm.createdAt;
    ...
    const userFarms = await Farm.findAll({
      where: {
        [Op.or]: [
          ...
          { '$farmInstallmentOwnerships.user_id$': requestedUserId },
          { '$farmUnitOwnerships.user_id$': requestedUserId }
        ]
      },
      ...
      return await Farm.findAll({
        where: {
          [Op.or]: [
            ...
            { 
              '$farmInstallmentOwnerships.user_id$': userId,
              '$farmInstallmentOwnerships.createdAt$': { [Op.lte]: date }
            },
            { 
              '$farmUnitOwnerships.user_id$': userId,
              '$farmUnitOwnerships.createdAt$': { [Op.lte]: date }
            }
          ]
        },
        include: [
